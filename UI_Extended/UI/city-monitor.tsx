...
const SchoolRow = ({ iconSrc, label, students, free, showText, compactValues }:
    { iconSrc: string; label: string; students: number; free: number; showText: boolean; compactValues: boolean }) => (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "3rem 0" }}>
        <span style={{ display: "flex", alignItems: "center", minWidth: 0 }}>
            <Tooltip tooltip={label}>
                <div style={{ display: "flex" }}>
        ...
        <span style={{ whiteSpace: "nowrap" }}>
            {compactValues ? (
                <span style={{ color: free <= 0 ? "#ff6b6b" : "#7CFC00" }}>{free}</span>
            ) : (
                <>
                    {students}{"\u00A0/\u00A0"}
                    <span style={{ color: free <= 0 ? "#ff6b6b" : "#7CFC00" }}>{free}</span>
...
export const CityMonitorComponent = () => {
    ...
    const save = (override: Partial<{ pos: { x: number; y: number }; minimized: boolean; visible: boolean; showText: boolean; width: number | null }> = {}) => {
        ...
        try { trigger("cityMonitor", "saveUiState", JSON.stringify(s)); } catch { }
    };
                        ...
                        {!minimized && (
                            <div style={{ padding: "10rem" }}>
                                ...
                                <Row
                                    ...
                                    value={compactValues ? String(data.openJobs) : `${data.openJobs} / ${data.totalJobSlots}`}
                                    showText={showText}
                                />

                                <div style={{ height: "1rem", background: "rgba(255,255,255,0.1)", margin: "8rem 0" }} />

                                <SchoolRow iconSrc={edu1} label="Grundschule" students={data.elementaryStudents} free={data.elementaryFreeSlots} showText={showText} compactValues={compactValues} />
                                <SchoolRow iconSrc={edu2} label="Oberschule" students={data.highStudents} free={data.highFreeSlots} showText={showText} compactValues={compactValues} />
                                <SchoolRow iconSrc={edu3} label="College" students={data.collegeStudents} free={data.collegeFreeSlots} showText={showText} compactValues={compactValues} />
                                <SchoolRow iconSrc={edu4} label="Universität" students={data.uniStudents} free={data.uniFreeSlots} showText={showText} compactValues={compactValues} />
                            </div>
                        )}
